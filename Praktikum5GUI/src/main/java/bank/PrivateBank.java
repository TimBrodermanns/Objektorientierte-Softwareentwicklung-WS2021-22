...
public class PrivateBank implements Bank{
    ...
    public void deleteAccount(String acc) throws AccountDoesNotExistException{
        if(!this.accountsToTransactions.containsKey(acc)) throw new AccountDoesNotExistException();
        this.accountsToTransactions.remove(acc);
        try {
            File f = new File(this.getPath() + "\\" + acc + ".json");
            f.delete();
            if(new File(this.getPath()+"\\" + acc + ".json").exists()) throw new Exception("Kritischer Fehler beim löschen");
        }catch (Exception e){
    ...
    public List<String> getAllAccounts(){
        return this.accountsToTransactions.keySet().stream().toList();
    }
